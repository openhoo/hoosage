...
function overview(calls: UsageCall[]) {
  ...
  if (allEmpty)
    return `<section class="onboarding">${icon("activity")}<span class="section-eyebrow">LOCAL USAGE</span><h2>${data.status === "waiting" ? "No usage yet" : data.status === "reload" ? "Reload required" : data.status === "blocked" ? "Tracking unavailable" : "Tracking is off"}</h2><p>${h(data.statusDetail)}</p><div class="onboarding-actions">${setupAction}<button class="button" data-action="diagnose">Diagnose tracking</button><button class="button" data-action="demo">Preview ${icon("arrow")}</button></div></section><div class="coverage-note"><span>Tracking starts after setup and reload. <button class="text-button" data-page="about">Usage details ${icon("arrow")}</button></span></div>`;
...
function projectDescription(project: Snapshot["projects"][number], hasUsage: boolean) {
  ...
  if (project.kind === "workspace")
    return `Workspace group · ${count(project.folderCount, "folder")}`;
  if (project.id === data!.currentProjectId) return "Current workspace";
  return hasUsage ? "Workspace" : "No usage in selected period";
}
...
function projects(calls: UsageCall[], embedded = false) {
  ...
  const projectName = (row: (typeof rows)[number], index: number) =>
    `<span class="folder-icon tone-${index % 4}">${icon("projects")}</span><span><strong>${h(row.project.name)}</strong><small>${h(projectDescription(row.project, row.usage.length > 0))}</small></span>`;
...
function about() {
  return `<section class="about-hero"><span class="about-hero-icon">${icon("about")}</span><div><span class="section-eyebrow">LOCAL BY DESIGN</span><h2>Clear numbers. Private work.</h2><p>Hoosage stores model names, usage counts, costs, timing and session IDs on the extension host. Prompts, responses, code and tool arguments are excluded.</p></div><div class="about-hero-chips"><span>On this host</span><span>No content stored</span></div></section>
